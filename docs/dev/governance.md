...
1. **OAuth 2.1 client credentials** — `POST /oauth/token` (form body). Clients: `architect`, `code-reviewer`, `adversarial-code-critic`, `sre`. Issues **RS256 JWTs** with 15-min TTL, signed with a private RSA key loaded from `JWT_PRIVATE_KEY_FILE`.
